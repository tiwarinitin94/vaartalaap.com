...
class user extends Models
{
    ...
    public static function check_if_post_exist($id)
    {
        ...
        if ($id) {
            ...
            $check = $db->select("*", "posts", " id='$id'") or die(mysqli_error($check));
    ...
    public static function get_specific_post($get_check)
    {
        ...
        if (isset($_POST['post_id'])) {
            ...
            $check = $db->select("*", "posts", " id='$last_id'") or die(mysqli_error($check));
        } else if ($get_check) {
            $check = $db->select("*", "posts", " id='$get_check'") or die(mysqli_error($check));
        }
    ...
    public static function get_user_news($get_check)
    {
        ...
        /*SELECT posts.id,posts.body,posts.date_added,posts.added_by,posts.user_posted_to,posts.post_share_snip,posts.if_shared,myusers.id,myusers.first_name,myusers.last_name, myusers.profile_pic  
        FROM posts INNER JOIN myusers ON myusers.id=posts.added_by*/
        ...
        if ($get_check == "helloalkddaklfjakldjaksdjaakdjdakljdkjdksjkjakjdksjdakjssjdksdjsa") {
            ...
            if ($last_id == "nothing") {
                $check = $db->select(
                    "posts.post_id,posts.body,posts.date_added,posts.added_by,posts.user_posted_to,posts.post_share_snip,posts.if_shared,myusers.id,myusers.first_name,myusers.last_name, myusers.profile_pic, myusers.username",
                    "posts INNER JOIN myusers ON myusers.id=posts.added_by",
					"(posts.added_by='$user' OR posts.user_posted_to='$user') 
					ORDER BY posts.post_id DESC LIMIT 15"
                ) or die(mysqli_error($check));
            } else {
                $check = $db->select(
                    "posts.post_id,posts.body,posts.date_added,posts.added_by,posts.user_posted_to,posts.post_share_snip,posts.if_shared,myusers.id,myusers.first_name,myusers.last_name, myusers.profile_pic",
                    "posts INNER JOIN myusers ON myusers.id=posts.added_by",
					"posts.post_id<'$last_id' AND (added_by='$user' OR user_posted_to='$user') 
            ...
            echo "If you are bad then I am your dad.";
        }
    }
    //--------------------------------------------news data end---------------------------------------
    ...
    private static function get_user_friends($user_pro = false)
    {
        ...
        $user = Session::get('user');
        if (!$user_pro) {
            $check = $db->select("friend_array", "myusers", "username='$user'") or die(mysqli_error($check));
        } else {
            $check = $db->select("friend_array", "myusers", "username='$user_pro'") or die(mysqli_error($check));
        }
        $check->setFetchMode(PDO::FETCH_ASSOC);
        $check->execute();
        $data = $check->fetch();
